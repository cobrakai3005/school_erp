...
export default function SuperAdminTeachers() {
  ...
  const fetchTeachers = async () => {
    ...
    try {
      ...
      const response = await superAdminApi.getAllTeachers(params);
      if (response.success) {
    ...
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-slate-600">View teachers across all schools</p>
        </div>
              ...
              <tbody className="divide-y divide-slate-200">
                {teachers.map((teacher) => (
                  <tr key={teacher.id} className="hover:bg-slate-50">
                    ...
                    <Td>
                      <span className="text-sm">{teacher.school_name || '-'}</span>
                    </Td>
                    <Td>
                      <span className="text-sm">{teacher.subjects || '-'}</span>
                    </Td>
                    <Td>{teacher.phone || '-'}</Td>
                    <Td>
                      <Badge
